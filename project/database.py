...
def load_environment() -> Path:
    ...
    _debug("Buscando archivo .env...")
    for candidate in ENV_CANDIDATES:
        exists = candidate.exists()
        _debug(f"  - {candidate} -> {'ENCONTRADO' if exists else 'no existe'}")
        if exists:
            loaded = load_dotenv(dotenv_path=candidate, override=True, encoding="utf-8-sig")
            _debug(f"load_dotenv() retorno: {loaded}")
...
def read_neo4j_config() -> dict:
    ...
    _debug("--- Variables despues de limpieza (.strip / comillas) ---")
    _debug(f"URI: {uri if uri else '(vacio)'}")
    ...
    _debug(f"USER: {user if user else '(vacio)'} (fuente: {user_source})")
    _debug(f"DATABASE: {database}")
    _debug(f"PASSWORD cargada: {'SI' if password else 'NO'}")
    if password:
        _debug(f"PASSWORD preview: {_mask_secret(password)}")
        _debug(f"PASSWORD longitud: {len(password)}")
    ...
    if not uri.startswith(("neo4j+s://", "neo4j+ssc://", "bolt+s://", "neo4j://", "bolt://")):
        _debug(f"ADVERTENCIA: URI con esquema inusual: {uri.split('://')[0]}://")
...
class Neo4jConnection:
    ...
    def _connect(self) -> None:
        _debug("--- Intentando conectar a Neo4j ---")
        _debug(f"URI final: {self.uri}")
        _debug(f"USER final: {self.user}")
        _debug(f"PASSWORD longitud final: {len(self.password)}")
        _debug(f"PASSWORD preview: {_mask_secret(self.password)}")
        ...
        try:
            ...
            _debug("Driver GraphDatabase creado correctamente.")
    ...
    def verify_connection(self) -> bool:
        ...
        _debug("Ejecutando verify_connectivity()...")
        try:
